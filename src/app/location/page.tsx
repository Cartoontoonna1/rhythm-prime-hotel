...
export default function LocationPage() {
  return (
    <main className="rp-shell"><SiteHeader />
      ...
      <section className="rp-section"><div className="rp-container rp-grid-2"><div><p className="rp-eyebrow">DESTINATION LOGIC</p><h2 className="rp-title" style={{fontSize:"clamp(2.4rem,4vw,4.3rem)"}}>A location designed around the guest journey.</h2><p className="rp-subtitle">This portfolio concept intentionally avoids claiming a real address or fabricated travel times. Instead, the location strategy demonstrates how a Phuket lifestyle hotel can communicate access, atmosphere and nearby experiences.</p><div className="rp-floor-strip">{points.map(([name,detail])=><div className="rp-floor-chip" key={name}><strong>{name}</strong><span>{detail}</span></div>)}</div></div><div className="rp-image rp-image--map"><Image src="/images/home/rhythm-prime-location-map.png.png" alt="Conceptual Rhythm Prime map showing airport, beach, Old Town and island landmarks" fill sizes="(max-width:720px) 100vw,50vw" /></div></div></section>
